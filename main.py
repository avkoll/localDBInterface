...
def natural_language_to_sql(query, model, tokenizer, max_length=256, max_new_tokens=50):
    """
    ...
    - max_length (int): Maximum length for the tokenized input.
    - max_new_tokens (int): Maximum number of new tokens to generate for the SQL query. Default is 50.
    ...
    try:
        ...
        # Generate the SQL query using the model, limiting new tokens generated
        outputs = model.generate(input_ids, attention_mask=attention_mask, max_new_tokens=max_new_tokens, num_beams=6, early_stopping=True)
